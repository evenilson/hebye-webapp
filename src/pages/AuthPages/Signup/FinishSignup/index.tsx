...
import { schemaFormFinishSignup } from "../../../../utils/schemasYup";
import { FormContainer } from "../../styles";

import { FinishSignupContainer, StepsContainer } from './style';

export function FinishSignup() {

  const { register, handleSubmit, formState: { errors }} = useForm({
    resolver: yupResolver(schemaFormFinishSignup),
  })
  ...
  const onSubmit = (data: any) => {
    ...
    console.log(data);
    updateProfile(data.firstName, data.lastName).finally(() => {setLoading(false)});
  }
...

      {
        stepCurrent === 1 ? (
          <>
            ...
            <ButtonSimple title="Continuar →" onclick={nextStep}/>
          </>
        ) : (
          <FormContainer onSubmit={handleSubmit(onSubmit)}>
            ...
            <Input 
              registerAndErros={ {'register': register, 'errors': errors} }
              name="lastName" title="Sobrenome" 
            />

            <Button title="Finalizar" isLoading={loading} />
